...
with col_cal:
    ...
    # Calendar — aria-label CSS works ONLY with single-line button labels (no newlines)
    ...
    def make_label(dn, pnl):
        sign = "+" if pnl >= 0 else ""
        return f"{dn}   {sign}\u00a3{abs(pnl):.0f}"

    css_parts = ["<style>"]
    css_parts.append("""
      div[data-testid="stHorizontalBlock"] button {
        min-height:72px !important; border-radius:10px !important;
        font-size:13px !important; font-weight:800 !important;
        width:100% !important; transition:transform 0.12s,box-shadow 0.12s !important;
      }
    """)
    for dn in range(1, days_in_month+1):
        ...
        rgb    = "15,110,86" if pnl>=0 else "192,57,43"
        shadow = f"0 4px 12px rgba({rgb},0.35)" if is_sel else f"0 2px 8px rgba({('95,204,168' if pnl>=0 else '232,131,108')},0.30)"
        ...
        css_parts.append(f"""
          button[aria-label="{lbl}"] {{
            ...
            color:{clr} !important; box-shadow:{shadow} !important; {out}
          }}
          button[aria-label="{lbl}"]:hover {{
            transform:translateY(-2px) !important; box-shadow:0 6px 16px rgba(0,0,0,0.18) !important;
          }}""")
    ...
    for row_start in range(0, len(all_days), 7):
        ...
        for ci, dn in enumerate(row):
            with cols[ci]:
                if dn is None:
                    st.markdown('<div style="min-height:74px;border:1px solid #f0f0f0;border-radius:10px;background:#fafafa;margin:2px 0"></div>', unsafe_allow_html=True)
                else:
                    pnl = day_pnl_map.get(dn, None)
                    dk  = f"{cal_year}-{str(cal_mon).zfill(2)}-{str(dn).zfill(2)}"
                    if pnl is not None:
                        lbl = make_label(dn, pnl)
                        if st.button(lbl, key=f"cal_{dk}", use_container_width=True):
                            st.session_state.selected_day = dk if st.session_state.selected_day != dk else None
                            st.rerun()
                    else:
                        st.markdown(f'<div style="min-height:74px;border:1px solid #eee;border-radius:10px;background:#fafafa;display:flex;align-items:flex-start;padding:8px;margin:2px 0"><span style="font-size:11px;color:#ccc;font-weight:600">{dn}</span></div>', unsafe_allow_html=True)
