...
with col_cal:
    ...
    # Calendar grid using Streamlit buttons — fully clickable
    # Inject CSS to style each button as a calendar cell
    st.markdown("""<style>
      /* DOW header */
      .cal-dow-header { display:grid; grid-template-columns:repeat(7,1fr); gap:4px; margin-bottom:4px; }
      .cal-dow-header div { text-align:center; font-size:10px; font-weight:700; color:#bbb; text-transform:uppercase; padding:2px 0; }
      /* All calendar buttons reset */
      div[data-testid="stHorizontalBlock"] div[data-testid="stButton"] button {
        border-radius:8px !important; min-height:62px !important; width:100% !important;
        font-size:11px !important; font-weight:800 !important; line-height:1.4 !important;
        white-space:pre-line !important; padding:4px 2px !important;
      ...
      .win-day button {
        ...
        box-shadow:0 2px 6px rgba(95,204,168,0.3) !important;
      }
      .win-day button:hover { transform:translateY(-2px) !important; box-shadow:0 4px 12px rgba(95,204,168,0.45) !important; }
      /* Loss day */
      .loss-day button {
        background:linear-gradient(135deg,#fde8e4,#f9c4bb) !important;
        border:1.5px solid #e8836c !important; color:#C0392B !important;
        box-shadow:0 2px 6px rgba(232,131,108,0.3) !important;
      }
      .loss-day button:hover { transform:translateY(-2px) !important; box-shadow:0 4px 12px rgba(232,131,108,0.45) !important; }
      /* Selected win */
      .win-sel button { outline:2.5px solid #0F6E56 !important; outline-offset:2px !important;
        background:linear-gradient(135deg,#a8ead0,#5fcca8) !important;
        box-shadow:0 4px 14px rgba(15,110,86,0.35) !important; }
      /* Selected loss */
      .loss-sel button { outline:2.5px solid #C0392B !important; outline-offset:2px !important;
        background:linear-gradient(135deg,#f9c4bb,#e8836c) !important;
        box-shadow:0 4px 14px rgba(192,57,43,0.35) !important; }
      /* Empty day */
      .empty-day button { background:#fafafa !important; border:1px solid #eee !important;
        color:#ddd !important; cursor:default !important; }
      .empty-day button:hover { transform:none !important; box-shadow:none !important; }
    ...
    # DOW header
    st.markdown('<div class="cal-dow-header">' + ''.join([f'<div>{d}</div>' for d in ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']]) + '</div>', unsafe_allow_html=True)
    ...
    for row_start in range(0, len(all_days), 7):
        ...
        for ci, dn in enumerate(row):
            with cols[ci]:
                if dn is None:
                    st.markdown('<div class="empty-day">', unsafe_allow_html=True)
                    st.button(" ", key=f"cal_empty_{row_start}_{ci}", disabled=True, use_container_width=True)
                    st.markdown('</div>', unsafe_allow_html=True)
                else:
                    ...
                    if pnl is not None:
                        ...
                        css  = ("win-sel" if is_sel else "win-day") if pnl>=0 else ("loss-sel" if is_sel else "loss-day")
                        ...
                        st.markdown('<div class="empty-day">', unsafe_allow_html=True)
                        st.button(str(dn), key=f"cal_{dk}_e", disabled=True, use_container_width=True)
                        st.markdown('</div>', unsafe_allow_html=True)

        # Track days with data for auto-select in col_detail
