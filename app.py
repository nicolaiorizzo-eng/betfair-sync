...
<style>
  ...
  /* metric cards — fixed height so all equal */
  .metric-card {
    background:#fff;
    border-radius:10px;
    padding:12px 8px;
  ...
  /* section card wrapping each chart/table */
  .card {
    background:#fff;
    border-radius:12px;
  ...
  /* calendar */
  .cal-day-wrap { position:relative; margin:2px 0; }
  .cal-day {
    border-radius:7px; padding:5px 2px; text-align:center;
    min-height:56px; border:1.5px solid #eee; background:#fff;
  }
  .cal-day.win  { background:linear-gradient(135deg,#d4f5e9,#a8ead0); border-color:#5fcca8; box-shadow:0 2px 5px rgba(95,204,168,0.25); }
  .cal-day.loss { background:linear-gradient(135deg,#fde8e4,#f9c4bb); border-color:#e8836c; box-shadow:0 2px 5px rgba(232,131,108,0.25); }
  .cal-day.selected.win  { outline:2.5px solid #0F6E56; outline-offset:1px; }
  .cal-day.selected.loss { outline:2.5px solid #C0392B; outline-offset:1px; }
  .cal-day-num  { font-size:10px; color:#999; font-weight:600; }
  .cal-day-pnl  { font-size:11px; font-weight:800; margin-top:1px; }
  .cal-day-bets { font-size:9px; color:#aaa; }
  ...
  /* hide streamlit button label on calendar */
  .stButton button { visibility:hidden; position:absolute; top:0; left:0; width:100%; height:100%; cursor:pointer; }
  .cal-day-wrap .stButton { position:absolute; top:0; left:0; width:100%; height:100%; }
</style>
...
metric_card(cols[2],"Avg ROI",          f"{avg_roi:+.1f}%",         "pos" if avg_roi>=0 else "neg")
...
# ── ROW 2: advanced metrics in 3 cards ────────────────────────────────────────
st.markdown("#### Advanced Analytics")
ac1, ac2, ac3 = st.columns(3)

with ac1:
    pf_color="#0F6E56" if profit_factor>=1 else "#C0392B"
    ev_color="#0F6E56" if ev_per_bet>=0 else "#C0392B"
    lw_color="#0F6E56"; ll_color="#C0392B"
    st.markdown(f'''<div class="card"><div class="card-title">💰 Profitability Metrics</div>
      <div class="stat-grid">
        {stat_item("Largest Win", f"+£{largest_win:.2f}", lw_color)}
        {stat_item("Largest Loss", f"£{largest_loss:.2f}", ll_color)}
        {stat_item("Profit Factor", f"{profit_factor:.2f}", pf_color)}
        {stat_item("EV per Bet", f"£{ev_per_bet:+.2f}", ev_color)}
        {stat_item("Avg Odds", f"{avg_odds:.2f}")}
        {stat_item("Total Bets", str(total_bets))}
      </div></div>''', unsafe_allow_html=True)

with ac2:
    dd_color="#C0392B" if max_dd<0 else "#0F6E56"
    st.markdown(f'''<div class="card"><div class="card-title">📉 Risk Metrics</div>
      <div class="stat-grid">
        {stat_item("Max Drawdown", f"£{max_dd:.2f}", dd_color)}
...
with ac3:
    wl_color="#0F6E56" if wl_ratio>=1 else "#C0392B"
    aw_color="#0F6E56"; al_color="#C0392B"
    st.markdown(f'''<div class="card"><div class="card-title">🎯 Win/Loss Breakdown</div>
      <div class="stat-grid">
        {stat_item("Avg Win", f"+£{avg_win:.2f}", aw_color)}
        {stat_item("Avg Loss", f"£{avg_loss:.2f}", al_color)}
        {stat_item("W/L Ratio", f"{wl_ratio:.2f}", wl_color)}
        {stat_item("Strike Rate", f"{strike_rate:.1f}%", "#0F6E56" if strike_rate>=50 else "#C0392B")}
        {stat_item("Total Won", f"+£{total_won:.2f}", aw_color)}
        {stat_item("Total Lost", f"-£{total_lost:.2f}", al_color)}
      </div></div>''', unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ── ROW 3: cumulative + risk vs return ────────────────────────────────────────
...
with col_cal:
    ...
    for i,d in enumerate(['Mon','Tue','Wed','Thu','Fri','Sat','Sun']):
        dow_cols[i].markdown(f'<div style="text-align:center;font-size:10px;font-weight:700;color:#bbb;padding:2px 0;text-transform:uppercase">{d}</div>',unsafe_allow_html=True)
    ...
    if total_cells%7!=0: total_cells+=7-(total_cells%7)

    cell_idx=0
    while cell_idx<total_cells:
        row_cols=st.columns(7)
        for ci in range(7):
            dn=cell_idx+ci-first_dow+1
            if 1<=dn<=days_in_month:
                ...
                with row_cols[ci]:
                    if pnl is not None:
                        bg='linear-gradient(135deg,#d4f5e9,#a8ead0)' if pnl>=0 else 'linear-gradient(135deg,#fde8e4,#f9c4bb)'
                        brd='#5fcca8' if pnl>=0 else '#e8836c'
                        clr='#0F6E56' if pnl>=0 else '#C0392B'
                        out=f'2.5px solid {"#0F6E56" if pnl>=0 else "#C0392B"}' if is_sel else 'none'
                        sh='0 4px 10px rgba(0,0,0,0.18)' if is_sel else '0 2px 5px rgba(0,0,0,0.10)'
                        pstr=f"{'+'if pnl>=0 else ''}£{abs(pnl):.0f}"
                        st.markdown(f'''<div style="position:relative;margin:2px 0">
                          <div style="background:{bg};border:1.5px solid {brd};border-radius:7px;
                            padding:5px 2px;text-align:center;min-height:56px;
                            outline:{out};outline-offset:1px;box-shadow:{sh}">
                            <div style="font-size:10px;color:#888;font-weight:600">{dn}</div>
                            <div style="font-size:11px;font-weight:800;color:{clr};margin-top:1px">{pstr}</div>
                            <div style="font-size:9px;color:#aaa">{bets}b</div>
                          </div>
                        </div>''',unsafe_allow_html=True)
                        if st.button(".",key=f"d_{dk}",help=f"Analyse {dn} {calendar.month_abbr[cal_mon]}",use_container_width=True):
                            st.session_state.selected_day=dk if st.session_state.selected_day!=dk else None
                            st.rerun()
                    else:
                        st.markdown(f'''<div style="border:1px solid #eee;border-radius:7px;padding:5px 2px;
                          text-align:center;min-height:56px;background:#fafafa;margin:2px 0">
                          <div style="font-size:10px;color:#ddd;font-weight:600">{dn}</div>
                        </div>''',unsafe_allow_html=True)
            else:
                with row_cols[ci]:
                    st.markdown('<div style="min-height:56px;margin:2px 0"></div>',unsafe_allow_html=True)
        cell_idx+=7
    st.markdown('</div>',unsafe_allow_html=True)

with col_detail:
    st.markdown('<div class="card" style="min-height:400px"><div class="card-title">📋 Day Analysis</div>', unsafe_allow_html=True)
    if st.session_state.selected_day:
        ...
        if not ddf.empty:
            ...
            st.markdown(f'''<div style="background:#f8f9fa;border-radius:8px;padding:12px;margin-bottom:10px;border:1px solid #eee">
              <div style="font-size:11px;color:#aaa;font-weight:600;text-transform:uppercase;margin-bottom:2px">{sel_date.strftime('%A, %d %B %Y')}</div>
              <div style="font-size:26px;font-weight:800;color:{dc}">{'+'if dt>=0 else ''}£{dt:.2f}</div>
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:10px">
                <div style="text-align:center;background:#fff;border-radius:6px;padding:6px;border:1px solid #eee">
                  <div style="font-size:9px;color:#aaa;font-weight:700;text-transform:uppercase">Bets</div>
                  <div style="font-weight:800;font-size:16px">{db}</div>
                </div>
                <div style="text-align:center;background:#fff;border-radius:6px;padding:6px;border:1px solid #eee">
                  <div style="font-size:9px;color:#aaa;font-weight:700;text-transform:uppercase">Wins</div>
                  <div style="font-weight:800;font-size:16px">{dw}/{db}</div>
                </div>
                <div style="text-align:center;background:#fff;border-radius:6px;padding:6px;border:1px solid #eee">
                  <div style="font-size:9px;color:#aaa;font-weight:700;text-transform:uppercase">ROI</div>
                  <div style="font-weight:800;font-size:16px;color:{dc}">{droi:+.1f}%</div>
                </div>
              </div>
            </div>''',unsafe_allow_html=True)
            for _,bet in ddf.iterrows():
                pc="#0F6E56" if bet['P/L']>=0 else "#C0392B"
                cls="win" if bet['P/L']>=0 else "loss"
                st.markdown(f'''<div class="bet-row {cls}">
                  <div style="font-weight:700;font-size:12px;color:#222">{bet['Event']}</div>
                  <div style="color:#888;font-size:11px;margin-top:1px">{bet['Market']} · {bet['Selection']} · {bet['Type']} @ {bet['Avg Odds']:.2f}</div>
        ...
        st.markdown('''<div style="text-align:center;padding:60px 20px;color:#ccc">
          ...
          <div style="font-size:13px;font-weight:500">Click any coloured day in the<br>calendar to see a full breakdown</div>
        </div>''',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ── ROW 5: market + day of week ───────────────────────────────────────────────
...
with r3c2:
    ...
    if not tw.empty:
        ...
        wt=tw['P/L'].sum()
        cap=f"Week {ws.strftime('%d %b')} – {we.strftime('%d %b %Y')} · Total: {'+'if wt>=0 else ''}£{wt:.2f}"
    else:
