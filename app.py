            ...
            with row_cols[ci]:
                if 1<=dn<=days_in_month:
                    ...
                    if pnl is not None:
                        ...
                        btn_color = "#0F6E56" if pnl>=0 else "#C0392B"
                        btn_bg    = "linear-gradient(135deg,#d4f5e9,#a8ead0)" if pnl>=0 else "linear-gradient(135deg,#fde8e4,#f9c4bb)"
                        btn_brd   = "#5fcca8" if pnl>=0 else "#e8836c"
                        outline   = f"outline:2.5px solid {btn_color};outline-offset:1px;" if is_sel else ""
                        uid = f"cal_{dk}".replace("-","_")
                        st.markdown(f"""<style>
                          div.{uid} button {{
                            background:{btn_bg} !important;
                            border:1.5px solid {btn_brd} !important;
                          ...
                          div.{uid} button:hover {{
                            transform:translateY(-1px);
                            box-shadow:0 4px 10px rgba(0,0,0,0.18) !important;
                          }}
                        </style>
                        <div class="{uid}">""", unsafe_allow_html=True)
                        if st.button(pstr, key=f"cal_{dk}", use_container_width=True, help=f"Analyse {dn} {calendar.month_abbr[cal_mon]}"):
                            st.session_state.selected_day = dk if st.session_state.selected_day!=dk else None
                            st.rerun()
                        st.markdown('</div>', unsafe_allow_html=True)
                    else:
                        st.button(str(dn), key=f"cal_{dk}_empty", disabled=True, use_container_width=True)
                else:
                    st.markdown('<div style="min-height:56px"></div>', unsafe_allow_html=True)
        cell_idx+=7
    st.markdown('</div>', unsafe_allow_html=True)

with col_detail:
    st.markdown('<div class="card" style="min-height:420px"><div class="card-title">📋 Day Analysis</div>', unsafe_allow_html=True)
