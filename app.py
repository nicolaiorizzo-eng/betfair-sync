...
with col_cal:
    ...
    # Calendar — clickable buttons with per-button scoped CSS via key attribute
    ...
    # Pre-build all per-button CSS in one <style> block
    css_rules = []
    ...
    for dn in range(1, days_in_month+1):
        ...
        if pnl is not None:
            ...
            if is_sel:
                bg  = "linear-gradient(135deg,#a8ead0,#5fcca8)" if pnl>=0 else "linear-gradient(135deg,#f9c4bb,#e8836c)"
                out = f"outline:2.5px solid {clr};outline-offset:2px;"
            else:
                out = ""
            uid = dk.replace("-","_")
            css_rules.append(f"""
              button[data-testid="baseButton-secondary"][kind="secondary"]:has(+ *),
              div[data-testid="stButton"] button[key="cal_{dk}"],
              div.cal_{uid} button {{
                ...
                min-height:62px !important; border-radius:8px !important;
                white-space:pre-line !important; line-height:1.5 !important;
                box-shadow:0 2px 6px rgba(0,0,0,0.08) !important; {out}
              }}
              div.cal_{uid} button:hover {{
    ...
    for row_start in range(0, len(all_days), 7):
        ...
        for ci, dn in enumerate(row):
            with cols[ci]:
                if dn is None:
                    st.markdown(f'<div style="min-height:66px;border:1px solid #f0f0f0;border-radius:8px;background:#fafafa;margin:2px 0"></div>', unsafe_allow_html=True)
                else:
                    pnl  = day_pnl_map.get(dn, None)
                    ...
                    if pnl is not None:
                        ...
                        st.markdown(f'<div class="cal_{uid}">', unsafe_allow_html=True)
