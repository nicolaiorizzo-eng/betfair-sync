...
with col_cal:
    ...
    # Day buttons — one per day with data
    days_with_data = sorted(day_pnl_map.keys())
    if days_with_data:
        st.markdown('<div style="font-size:11px;color:#999;font-weight:600;margin:8px 0 6px;text-transform:uppercase;letter-spacing:0.5px">👆 Click a day to analyse</div>', unsafe_allow_html=True)
        btn_cols = st.columns(min(len(days_with_data), 7))
        for i, dn in enumerate(days_with_data[:7]):
            pnl = day_pnl_map[dn]
            dk  = f"{cal_year}-{str(cal_mon).zfill(2)}-{str(dn).zfill(2)}"
            is_sel = (st.session_state.selected_day == dk)
            pstr = f"{str(dn).zfill(2)} {'+'if pnl>=0 else ''}£{abs(pnl):.0f}"
            with btn_cols[i]:
                if st.button(pstr, key=f"daybtn_{dk}", use_container_width=True,
                             type="primary" if is_sel else "secondary"):
                    st.session_state.selected_day = dk if st.session_state.selected_day != dk else None
                    st.rerun()
        if len(days_with_data) > 7:
            btn_cols2 = st.columns(min(len(days_with_data)-7, 7))
            for i, dn in enumerate(days_with_data[7:14]):
                pnl = day_pnl_map[dn]
                dk  = f"{cal_year}-{str(cal_mon).zfill(2)}-{str(dn).zfill(2)}"
                is_sel = (st.session_state.selected_day == dk)
                pstr = f"{str(dn).zfill(2)} {'+'if pnl>=0 else ''}£{abs(pnl):.0f}"
                with btn_cols2[i]:
                    if st.button(pstr, key=f"daybtn2_{dk}", use_container_width=True,
                                 type="primary" if is_sel else "secondary"):
                        st.session_state.selected_day = dk if st.session_state.selected_day != dk else None
                        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
