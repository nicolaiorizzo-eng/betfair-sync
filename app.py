...
with col_cal:
    ...
    # Calendar — HTML cards for display, real buttons underneath for clicking
    ...
    # Global CSS: shrink the real buttons to be tiny click targets BELOW each card
    st.markdown("""<style>
      .cal-btn-row button {
        ...
        for ci, dn in enumerate(row):
            with cols[ci]:
                if dn is None:
                    ...
                    if pnl is not None:
                        bg  = ("linear-gradient(135deg,#a8ead0,#5fcca8)" if is_sel else "linear-gradient(135deg,#d4f5e9,#a8ead0)") if pnl>=0 \
                              else ("linear-gradient(135deg,#f9c4bb,#e8836c)" if is_sel else "linear-gradient(135deg,#fde8e4,#f9c4bb)")
                        brd = ("#0F6E56" if is_sel else "#5fcca8") if pnl>=0 else ("#C0392B" if is_sel else "#e8836c")
                        clr = "#0F6E56" if pnl>=0 else "#C0392B"
                        rgb = ("15,110,86" if is_sel else "95,204,168") if pnl>=0 else ("192,57,43" if is_sel else "232,131,108")
                        shadow = f"0 4px 12px rgba({rgb},{'0.40' if is_sel else '0.28'})"
                        outline = f"outline:2.5px solid {clr};outline-offset:2px;" if is_sel else ""
                        sign = "+" if pnl>=0 else "-"
                        amt  = f"{sign}\u00a3{abs(pnl):.0f}"
                        # CSS: pull the button up to overlap the card
                        st.markdown(f"""<style>
                          div[data-testid="stButton"]:has(button[aria-label="{{}}"]) {{{{
                            margin-top:-78px !important; height:74px !important;
                            position:relative !important; z-index:5 !important;
                          }}}}
                          div[data-testid="stButton"]:has(button[aria-label="{{}}"]) button {{{{
                            height:74px !important; min-height:74px !important;
                            opacity:0 !important; cursor:pointer !important;
                          }}}}
                        </style>""".format(amt, amt), unsafe_allow_html=True)
                        # HTML card — visual only
                        st.markdown(f"""<div style="background:{bg};border:1.5px solid {brd};
                            border-radius:10px;box-shadow:{shadow};{outline}padding:8px 10px;
                            height:72px;display:flex;flex-direction:column;
                            justify-content:space-between;margin-bottom:2px;cursor:pointer">
                          <div style="font-size:11px;font-weight:700;color:{clr};opacity:0.75">{dn}</div>
                          <div style="font-size:16px;font-weight:900;color:{clr};line-height:1">{amt}</div>
                        </div>""", unsafe_allow_html=True)
                        # Invisible overlaid button for click
                        if st.button(amt, key=f"cal_{dk}", use_container_width=True):
                            st.session_state.selected_day = dk if not is_sel else None
                            st.rerun()
                    else:
                        st.markdown(f"""<div style="
                            border:1px solid #eee;border-radius:10px;
                            background:#fafafa;height:72px;
                            display:flex;align-items:flex-start;
                            padding:8px 10px;margin-bottom:2px">
                          <span style="font-size:11px;color:#ccc;font-weight:600">{dn}</span>
                        </div>""", unsafe_allow_html=True)
