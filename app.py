...
with col_cal:
    ...
    # Build full HTML calendar — no buttons, pure HTML for display
    dow_labels = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
    header_html = ''.join([f'<th style="text-align:center;font-size:10px;font-weight:700;color:#bbb;padding:4px 0;text-transform:uppercase;width:14.28%">{d}</th>' for d in dow_labels])
    ...
    for dn in range(1, days_in_month+1):
        ...
        if pnl is not None:
            ...
            shadow = '0 3px 8px rgba(0,0,0,0.18)' if is_sel else '0 2px 5px rgba(0,0,0,0.08)'
            outline= f'outline:2.5px solid {clr};outline-offset:2px;' if is_sel else ''
            pstr   = f"{'+'if pnl>=0 else ''}£{abs(pnl):.0f}"
            cell   = f'''<td style="padding:3px">
              <div style="background:{bg};border:1.5px solid {brd};border-radius:8px;
                padding:6px 2px;text-align:center;min-height:58px;{outline}
                box-shadow:{shadow};cursor:pointer">
                <div style="font-size:10px;color:#888;font-weight:700">{dn}</div>
                <div style="font-size:13px;font-weight:800;color:{clr};margin-top:2px">{pstr}</div>
                <div style="font-size:9px;color:#aaa;margin-top:1px">{bets} bet{"s" if bets!=1 else ""}</div>
              </div></td>'''
        else:
            cell = f'''<td style="padding:3px">
              <div style="border:1px solid #eee;border-radius:8px;padding:6px 2px;
                text-align:center;min-height:58px;background:#fafafa">
    ...
    st.markdown(f'''<table style="width:100%;border-collapse:separate;border-spacing:0;margin-bottom:12px">
      <thead><tr>{header_html}</tr></thead>
      <tbody>{rows_html}</tbody>
    </table>''', unsafe_allow_html=True)

    # Day picker — clean selectbox to select day and trigger analysis
    days_with_data = sorted(day_pnl_map.keys())
    if days_with_data:
        st.markdown('<div style="font-size:11px;color:#999;font-weight:600;margin-bottom:4px;text-transform:uppercase">Select day to analyse</div>', unsafe_allow_html=True)
        day_options = {dn: f"{str(dn).zfill(2)} {calendar.month_name[cal_mon][:3]} {cal_year}  {'  +'if day_pnl_map[dn]>=0 else '  '}£{abs(day_pnl_map[dn]):.2f}" for dn in days_with_data}
        chosen_dn = st.selectbox("Pick day", options=list(day_options.keys()),
                                  format_func=lambda d: day_options[d],
                                  index=len(days_with_data)-1,
                                  label_visibility="collapsed")
        chosen_dk = f"{cal_year}-{str(cal_mon).zfill(2)}-{str(chosen_dn).zfill(2)}"
        if st.session_state.selected_day != chosen_dk:
            st.session_state.selected_day = chosen_dk
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
