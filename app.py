        ...
        for ci, dn in enumerate(row):
            with cols[ci]:
                if dn is None:
                    ...
                    if pnl is not None:
                        # Day has data (🟢 or 🔴 flags background style hook via aria-label)
                        dot = '🟢' if pnl >= 0 else '🔴'
                        sign = '+' if pnl >= 0 else '-'
                        formatted_pnl = f"{sign}£{int(abs(pnl))}"
                        
                        # Markdown text formatting using linebreaks (\n) to control layout position cleanly
                        lbl = f"{dn}\n\n{formatted_pnl}\n\n{dot}"
                        ...
                        # Day has NO data (⚪ flags neutral background style hook via aria-label)
                        lbl = f"{dn}\n\n—\n\n⚪"
